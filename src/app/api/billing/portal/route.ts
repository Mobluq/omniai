...
import { billingPortalSchema } from "@/lib/validators/api-schemas";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const body = billingPortalSchema.parse(await request.json());
    await assertWorkspaceAccess(user.id, body.workspaceId, "admin");
    const result = await new BillingService().createCustomerPortalSession(body.workspaceId);
