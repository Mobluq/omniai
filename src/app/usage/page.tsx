      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent className="text-sm text-muted-foreground">
            {formatNumber(summary.tokenInputEstimate)} in / {formatNumber(summary.tokenOutputEstimate)} out
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle className="text-2xl">
              {summary.requestCount ? Math.round((summary.successCount / summary.requestCount) * 100) : 0}%
            </CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            ...
            <CardContent className="grid gap-3">
              {summary.byProvider.length ? (
                summary.byProvider.map((provider) => (
                  <div key={provider.key} className="grid gap-2 rounded-md border p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div className="font-medium capitalize">{provider.key}</div>
                      <Badge className="bg-muted">{formatNumber(provider.requestCount)} requests</Badge>
                    </div>
                    <div className="grid gap-2 text-sm text-muted-foreground md:grid-cols-3">
                      <span>{formatNumber(provider.tokenInputEstimate + provider.tokenOutputEstimate)} tokens</span>
            ...
            <CardContent className="grid gap-3">
              {summary.byModel.length ? (
                summary.byModel.map((model) => (
                  <div key={model.key} className="grid gap-2 rounded-md border p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <div className="font-medium">{model.modelId}</div>
                        <div className="text-sm capitalize text-muted-foreground">{model.provider}</div>
                      </div>
            ...
            <CardContent className="grid gap-3">
              {summary.byRequestType.length ? (
                summary.byRequestType.map((requestType) => (
                  <div key={requestType.key} className="flex items-center justify-between rounded-md border p-3">
                    <span className="text-sm font-medium">{requestType.requestType.replaceAll("_", " ")}</span>
                    <span className="text-sm text-muted-foreground">{formatNumber(requestType.requestCount)}</span>
                  </div>
                ))
              ) : (
